const { Employee, Department, Position, User, Branch, Company, LeaveBalance, Attendance, Leave, AuditLog } = require('../models');
  ...
  try {
    ...
    const recentActivities = await AuditLog.findAll({
      where: {
        ...
        entityType: { [Op.in]: ['Attendance', 'Leave'] },
        action: { [Op.in]: ['CLOCK_IN', 'CLOCK_OUT', 'REGULARIZATION_REQUEST', 'LEAVE_APPLICATION_SUBMITTED'] },
      },
    ...
    const formattedActivities = recentActivities.map(activity => {
      ...
      } else if (activity.action === 'LEAVE_APPLICATION_SUBMITTED') {
        type = 'primary';
...
exports.getEmployeeDashboardStats = async (req, res) => {
  try {
    ...
    // 4. Fetch Pending Attendance Regularization Requests (if the field exists)
    let pendingRegularizationRequests = 0;
    try {
      pendingRegularizationRequests = await Attendance.count({
        where: {
          employeeId: employee.id,
      ...
      // If the fields don't exist, just use 0
      console.log('Regularization fields not found in Attendance model');
    }

    const pendingRequests = pendingLeaveRequests + pendingRegularizationRequests;

    // 5. Calculate attendance rate for current month
    const totalWorkingDays = moment().date(); // Days passed in current month
    const attendanceRate = totalWorkingDays > 0 ? ((attendanceThisMonth / totalWorkingDays) * 100).toFixed(1) : 0;
    ...
    res.json({
      ...
      data: {
        ...
        upcomingHolidays: 2, // Static for now
      },
    });
