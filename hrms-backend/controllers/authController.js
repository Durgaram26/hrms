...
exports.login = async (req, res) => {
  try {
    ...
    if (!employeeId) {
      message = "Login successful, but no employee profile found for this user. Attendance and some features may be restricted.";
    }
    
    res.json({ token, role: user.role, employeeId: employeeId, message });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
